// 「数据」页：原「统计」+「进度」两个 Tab 合并。
// 四个子 Tab：概览 / 薄弱词 / 错题 / 建议。
// 概览数据源与教师端学员详情同源（buildStudentProgressSummary 等）。
...
import type { Word, UserWordProgress, WordDefinition, WordPhrase, WordExample } from '@/lib/data';
...
import { CheckinCard, AZList, WrongList } from '@/components/StudentProgressParts';
...
type Section = 'overview' | 'weak' | 'wrong' | 'advice';
...
export default function DataScreen() {
  ...
  const weakIds = useMemo(
    () => new Set(weakEntries.map((w) => String(w.word_id))),
    [weakEntries],
  );
  ...
  const load = useCallback(async () => {
    ...
    const now = Date.now();
    try {
      const [s, weakIdsRaw, tc, ds, words, summ, weak, wrong, az] = await Promise.all([
      ...
      const weakSet = new Set(weakIdsRaw);
  ...
  const handleReviewGrade = async (grade: (typeof GRADES)[number]['grade']) => {
      ...
      <View style={[styles.container, { backgroundColor: colors.background, paddingTop: insets.top }]}>
        <View style={styles.contentCol}>
          <View style={styles.trainHeader}>
            ...
            <Text style={[styles.title, { color: colors.text, paddingHorizontal: 0 }]}>
          ...
          {reviewDone ? (
            <View style={styles.trainDoneWrap}>
              ...
              <TouchableOpacity style={[styles.trainDoneBtn, { backgroundColor: colors.tint }]} onPress={exitTraining}>
                <Text style={[styles.trainDoneBtnText, { color: colors.onTint }]}>返回数据</Text>
              </TouchableOpacity>
            </View>
          ) : reviewCurrent ? (
  ...
  // ===== 主界面 =====
  if ((loading && !summary && !stats) || !wordbook) {
    return (
      <View style={[styles.container, { backgroundColor: colors.background, paddingTop: insets.top }]}>
        <View style={styles.contentCol}>
          <Text style={[styles.title, { color: colors.text }]}>数据</Text>
      ...
      <View style={styles.contentCol}>
        ...
        {(isAdmin || isTeacher) && (
          <TouchableOpacity
            ...
            activeOpacity={0.7}
          >
            <FontAwesome name="users" size={15} color={colors.onTint} />
            <Text style={[styles.teacherBtnText, { color: colors.onTint }]}>学员学习情况</Text>
          </TouchableOpacity>
        )}
            ...
            <View style={{ gap: 12 }}>
              ...
              <View style={[styles.card, { backgroundColor: colors.card }]}>
                ...
                <View style={styles.statRow}>
                  ...
                  <View style={styles.statCell}>
                    <Text style={[styles.statNum, { color: stats && stats.due > 0 ? colors.danger : colors.text }]}>{stats?.due ?? 0}</Text>
              ...
              <AZList summary={summary} azWords={azWords} weakIds={weakIds} hasWordbook colors={colors} />
            </View>
          </ScrollView>
        )}
          ...
          <ScrollView contentContainerStyle={styles.sectionScroll}>
            <View style={[styles.card, { backgroundColor: colors.card }]}>
              ...
              {weakWords.length === 0 ? (
                ...
                <>
                  <Text style={[styles.weakHint, { color: colors.subtitle }]}>
                  ...
                  <View style={styles.trainBtnRow}>
                    <TouchableOpacity style={[styles.trainBtn, { backgroundColor: colors.tint }]} onPress={startReviewTraining}>
                      <FontAwesome name="refresh" size={13} color={colors.onTint} />
                      <Text style={[styles.trainBtnText, { color: colors.onTint }]}>专项复习</Text>
                    </TouchableOpacity>
                    <TouchableOpacity style={[styles.trainBtn, { backgroundColor: colors.tint }]} onPress={() => setTraining('quiz')}>
                      <FontAwesome name="pencil" size={13} color={colors.onTint} />
                      <Text style={[styles.trainBtnText, { color: colors.onTint }]}>专项练习</Text>
                    </TouchableOpacity>
                  </View>
                  {weakWords.map((w) => {
                      ...
                      <React.Fragment key={w.id}>
                        <TouchableOpacity style={[styles.weakItem, { backgroundColor: colors.background }]} onPress={() => handleWordPress(w)}>
                          ...
                          <View style={styles.weakRight}>
                            {p && p.wrong > 0 ? (
                              <Text style={[styles.weakTag, { color: colors.danger }]}>错 {p.wrong} 次{wrongPct >= 20 ? ` · ${wrongPct}%` : ''}</Text>
                            ) : p && p.due <= Date.now() - 3 * DAY ? (
                              <Text style={[styles.weakTag, { color: colors.danger }]}>逾期 {Math.floor((Date.now() - p.due) / DAY)} 天</Text>
                            ) : p && p.repetitions <= 1 ? (
                              <Text style={[styles.weakTag, { color: colors.danger }]}>未巩固</Text>
                            ) : null}
        ...
        {section === 'advice' && (
          <ScrollView contentContainerStyle={styles.sectionScroll}>
            <View style={[styles.card, { backgroundColor: colors.card }]}>
              <Text style={[styles.cardTitle, { color: colors.text }]}>学习建议</Text>
              {advice.length === 0 ? (
                <Text style={[styles.weakEmpty, { color: colors.subtitle }]}>✅ 暂无建议，保持当前节奏即可</Text>
              ) : advice.map((a, i) => (
...
const styles = StyleSheet.create({
  container: { flex: 1, paddingHorizontal: Layout.pagePadding },
  // 桌面宽屏下内容居中限宽
  contentCol: {
    flex: 1,
    width: '100%',
  ...
  teacherBtnText: { fontSize: 15, fontWeight: '700' },
  tabRow: {
    flexDirection: 'row',
    borderBottomWidth: 1,
    marginBottom: 12,
  ...
  sectionScroll: { paddingBottom: 40 },
  card: { borderRadius: Layout.radius.lg, padding: 16, marginBottom: 12 },
  ...
  todayBox: { flexDirection: 'row', justifyContent: 'space-between', borderRadius: Layout.radius.md, paddingHorizontal: 12, paddingVertical: 10, marginTop: 14 },
  todayItem: { fontSize: 13 },
  summaryCard: { borderRadius: Layout.radius.lg, padding: 14, marginBottom: 10 },
  summaryTitle: { fontSize: 15, fontWeight: '700' },
  summaryHint: { fontSize: 12, marginTop: 2 },
  weakEmpty: { fontSize: 14, paddingVertical: 8 },
  weakHint: { fontSize: 12, marginBottom: 10 },
  trainBtnRow: { flexDirection: 'row', gap: 10, marginBottom: 12 },
  trainBtn: { flex: 1, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 6, paddingVertical: 10, borderRadius: Layout.radius.md },
  trainBtnText: { fontSize: 14, fontWeight: '700' },
  weakItem: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', borderRadius: Layout.radius.md, paddingHorizontal: 12, paddingVertical: 10, marginBottom: 8 },
  ...
  weakTag: { fontSize: 11, fontWeight: '600' },
  detailCard: { marginTop: -6, marginBottom: 8, marginHorizontal: 4, borderRadius: Layout.radius.md, borderWidth: 1, padding: 12 },
  ...
  adviceItem: { flexDirection: 'row', alignItems: 'center', gap: 10, borderRadius: Layout.radius.md, paddingHorizontal: 12, paddingVertical: 11, marginTop: 8 },
  ...
  trainDoneBtnText: { fontWeight: '700', fontSize: 15 },
});
