...
## 22. 已部署（2026-07-29）：学习进度面板与智能练习

提交：`38fa7c5 fix: advance extra practice progress before phrase cards`，已合并 `main` 并部署到生产。
...
## 29. 修复（2026-08-11）：学习设置手机布局挤压（`88e52f7`，已部署）

**问题**：「我的-学习设置」在手机上挤在一起。根因：`goalCard` 是 `flexDirection: 'row'`
横向布局（早期只有一项设置），现在含 4 项设置（新词目标/词组总量/显示每日计划/目标完成日期）
全部挤在一行。

**修复**（`app/(tabs)/profile.tsx`）：
- goalCard 改纵向列布局（圆角带边框卡片），每项设置独占一行：左标签 + 右控件，行间 hairline 分隔
- 数字输入保持 90px 右对齐；日期行右侧显示当前值 + 日历图标
- 移除废弃的 planToggle 样式

## 30. 功能（2026-08-11）：薄弱词判定口径升级——"历史上掌握不牢靠"（`6503c01`，已部署）

**需求**：「需加强的词」应覆盖历史上掌握不牢靠的词，而非仅近期答错的词。

**判定维度**（`lib/data/weak.ts`，满足任一即为薄弱）：
1. 错误率 >= 34% 或 EF < 1.8（原有）
2. 近 30 天练习/复习错 >= 2 次（原有）
3. **逾期未复习（新增）**：`due <= now - 3天`——到期超 3 天仍未复习，记忆正在衰减
4. **低强度陈旧词（新增）**：首学已超 7 天但 `repetitions <= 1`——从未真正进入复习循环
   （典型场景：反复评 Again，due 被不断重置、看起来不逾期，但始终记不住）

**实现要点**：
- `listStudyLogs` 改为全量拉取（原 30 天窗口），内存中分别计算：
  30 天练习错误次数 + 每词首学时间（最早日志 ts）
